...
constexpr const wchar_t* PLUGIN_INFO = L"DepthMapFilter v1.1.0 azurite";
...
{
    if (!g_ort) {
        g_log_handle->error(g_log_handle, L"Failed to load onnxruntime.dll");
        return false;
    }
    ...
    if (!g_initialized) {
        std::filesystem::path model_path = g_app_data_path / L"Plugin" / std::wstring{PLUGIN_NAME} / L"depth_anything_v2_vits.onnx";
        if (!std::filesystem::exists(std::filesystem::status(model_path))) {
            g_log_handle->error(g_log_handle, L"The model file does not exist");
            return false;
        }
        try {
            ...
            g_log_handle->error(g_log_handle, L"Failed to load the model");
            return false;
        }
    }
    ...
    if (g_initialized) {
        ...
        try {
            ...
            g_log_handle->error(g_log_handle, L"Failed to run inference");
            return false;
        }
        return true;
    }
